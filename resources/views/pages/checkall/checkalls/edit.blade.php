...
@section('content')
    <div class="container-fluid" style="background: #f4f4f4;">
        <div class="row">
            <form class="mega-vertical" action="{{route('checkalls.update',$checkall->id)}}" method="post"
                                        ...
                                        <div class="mb-3 row">
                                            ...
                                            <select class="form-select digits" id="exampleFormControlSelect9" name="type_id" value="{{old('type_id')}}">
                                                        ...
                                                        @foreach($types as $item)
                                                          <option value='{{$item->id}}' {{$item->id==$checkall->type->id ? 'selected':''}}>{{$item->name}}</option>
                                                        @endforeach
                                ...
                                <div class="col-sm-12">
                                    <div class="media-body">
                                        <div class="mb-3 row">
                                            ...
                                            <div class="col-sm-9">
                                                <textarea name='question' rows="5" class="form-control">{{old('question',$checkall->question)}}</textarea>
                                        ...
                                        <div class="mb-3 row">
                                            ...
                                            <div class="col-sm-9">
                                                <input type="number" name='order'
                                                       value="{{old('order',$checkall->order==0?'':$checkall->order)}}" class="form-control"
                                                       placeholder="">
                                                @error('order')
                                                <div style='color:red'>{{$message}}</div>
                                                @enderror
